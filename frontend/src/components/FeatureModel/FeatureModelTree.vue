<template>
  <div>
    <feature-model-tree-toolbar @search="onChangeSearch" @coloring="onChangeColoring" @fitToView="zoomFit"
      @resetView="(levels, maxChildren) => resetView(levels, maxChildren)" @shortName="onChangeShortName"
      @verticalSpacing="onChangeVerticalSpacing" @export="$emit('exportToXML')"></feature-model-tree-toolbar>
    <div id="svg-container"></div>

    <feature-model-tree-context-menu :d3Node="contextMenuD3Node" :d3NodeEvent="contextMenuD3NodeEvent" @collapse="
      (d3Node) => {
    ...
    " @hideLeftSiblings="(d3Node) => hideLeftSiblings(d3Node)"
      @hideRightSiblings="(d3Node) => hideRightSiblings(d3Node)" @hideCurrentNode="(d3Node) => hideCurrentNode(d3Node)"
      @edit="(d3Node) => (editD3Node = d3Node)" @close="contextMenuD3Node = undefined"
      @add="(d3Node) => (d3ParentOfAddNode = d3Node)"></feature-model-tree-context-menu>
    ...
    <feature-model-tree-add-dialog :parent="d3ParentOfAddNode ? d3ParentOfAddNode.data : undefined"
      @close="d3ParentOfAddNode = undefined" @add="(newNode) => addNode(newNode)"></feature-model-tree-add-dialog>
  </div>
</template>
