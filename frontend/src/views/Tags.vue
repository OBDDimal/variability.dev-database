<template>
    <div>
        ...
        <v-data-table :headers="headers" :items="$store.state.tags" :loading="loading" class="elevation-1"
            :search="search">
            <template v-slot:top>
                <v-toolbar flat>
                    ...
                    <v-dialog v-model="dialog" max-width="500px">
                        ...
                        <v-card>
                            ...
                            <v-card-actions>
                                ...
                                <v-btn color="blue darken-1" text @click="save" :loading="addLoading">
                                    Save
                                </v-btn>
                            </v-card-actions>
                        </v-card>
                    ...
                    <v-dialog v-model="dialogDelete" max-width="500px">
                        <v-card>
                            ...
                            <v-card-actions>
                                ...
                                <v-spacer></v-spacer>
                                <v-btn color="primary" text @click="deleteItemConfirm" :loading="removeLoading">Delete</v-btn>
                                <v-spacer></v-spacer>
                            </v-card-actions>
                        </v-card>
                    </v-dialog>
            ...
            <template v-slot:item.actions="{ item }">
                ...
                <v-btn small rounded color="error" class="mr-2" @click="deleteItem(item)"
                    :disabled="item.owner === false">
    ...
    methods: {
        ...
        deleteItemConfirm() {
            ...
            api.delete(`${API_URL}tags/${this.editedID}/`)
                .then(() => {
                    this.$store.commit('updateSnackbar', { message: "Tag deleted successfully!", variant: "success", timeout: 5000, show: true })
                    this.$store.dispatch('fetchTags');
                    this.removeLoading = false
                })
                .catch((error) => {
                    this.$store.commit('updateSnackbar', { message: "Error: " + error.message, variant: "error", timeout: 5000, show: true })
                    this.removeLoading = false
                });
            this.closeDelete();
        },
        ...
        addTag() {
            this.addLoading = true,
                api
                    ...
                    .then(() => {
                        this.$store.commit('updateSnackbar', { message: "Tag added successfully!", variant: "success", timeout: 5000, show: true })
                        this.$store.dispatch('fetchTags');
