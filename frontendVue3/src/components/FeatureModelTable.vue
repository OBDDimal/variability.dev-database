      ...
      >
        ...
        <template #item="{item}" >
          <tr @mouseenter="HandleMouseover(item.raw.id)"
            ...
            <td>
              <div v-if="showAllTags || item.showTags|| item.raw.tags.length <= 2">
                <v-chip
              ...
              <div v-else>
                <v-chip class="ma-1">
                  {{ item.raw.tags[0].label }}
                </v-chip>
                <v-chip class="ma-1">
                  {{ item.raw.tags[1].label }}
                </v-chip>
                <span class="ma-1" @click="item.showTags=true">+ {{ item.raw.tags.length - 2 }}</span>
                <v-icon
                  class="expand-tags-button"
